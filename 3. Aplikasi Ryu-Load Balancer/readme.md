# Membuat Aplikasi Ryu-Server Load Balancer
1. Masuk pada instance EC2 yang telah terinstall Mininet.
```sh
ssh -i .ssh/labsuser.pem ubuntu@ip_instance
```
2. Membuat file topology `topo_lb.py` dengan perintah `nano` untuk membuat topologi yang terdiri atas 4 hosts (h1 - h4) dan 1 switch (s1) dengan topologi sebagai berikut:
...
if __name__ == '__main__':
    ...
    run()
```
3. Membuat file program ryu-load balancer `rr_lb.py` dengan perintah `nano` lalu isikan dengan kode di bawah. File ini akan menjalankan:
...
```
4. Menjalankan dua terminal menggunakan tmux.
```sh
tmux
```
> Untuk menjalankan dua terminal tekan 'Ctrl+b %'.
> Untuk berpindah terminal tekan 'Ctrl+b o'.
5. Pada terminal 1 jalankan mininet menggunakan custom topology `topo_lb.py` yang telah dibuat.
```sh
sudo python3 topo_lb.py
```
6. Pada terminal 2 jalankan program ryu-load balancer `rr_lb.py` yang telah dibuat.
```sh
...
```
8. Melakukan percobaan dengan `dump-flow`.
```sh
