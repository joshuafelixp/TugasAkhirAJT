# Membuat Aplikasi Ryu-Shortest Path Routing (SPF)
1. Masuk pada instance EC2 yang telah terinstall Mininet.
```sh
ssh -i .ssh/labsuser.pem ubuntu@ip_instance
```
2. Membuat file topology `topo-spf_lab.py` dengan perintah `nano'. Isikan dengan kode berikut.
```sh
#!/usr/bin/env python
...
```
3. Membuat file program ryu-SPF `dijkstra_Ryu_controller.py` dengan perintah `nano` lalu isikan dengan kode berikut.
```sh
...
5. Pada terminal 1 jalankan mininet menggunakan custom topology `topo-spf_lab.py` yang telah dibuat.
```sh
sudo python3 topo-spf_lab.py
```
6. Pada terminal 2 jalankan program ryu-spf `dijkstra_Ryu_controller.py` yang telah dibuat.
```sh
